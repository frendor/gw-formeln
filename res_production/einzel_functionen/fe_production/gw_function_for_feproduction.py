...
RES_ART = "Eisen"
RES_SHORT = "Fe"
filename_1x = "gw_uni4_eiseninfo"
filename_6x = "gw_speed6uni_eisenprod"
...
osc_initial_values = np.array([   0,  -16,  -64,   82, -102,  
                                110, -104,   14,  -60, -100, 
                                120,  -12,   52,   38,  -78,  
                                -70,  -90,  -50,   26,  114,  
                                -60,   62, -142,   54, -124])
    
osc_initial_slope = {0: 10.,
                     1: 22.,
                     2: -2.,
                     3:-12.,
                     4: -8.}
...
base_func = lambda stufe, speed=1: speed * 8/5. * 3*( stufe**2 + 1/250. * stufe**3 )


linear_split_func = lambda stufe, speed=1:  speed * 8/5. * ( 1 + stufe/250.) * {0: 5,
                                                                                1: 7,
                                                                                2: 8,
                                                                                3: 8,
                                                                                4: 7}[stufe%5]

res_function = lambda stufe, speed=1: np.round(base_func(stufe,speed) + linear_split_func(stufe,speed) + osc_func(stufe,speed), decimals = 6)
res_diff_function = lambda stufe, value,speed=1: np.round(value - base_func(stufe,speed) - linear_split_func(stufe,speed) - osc_func(stufe,speed), decimals = 6)
...
def plot_resproduction(savefig=False, res_line = orig_values,
    ...
    if show_absolut_plots:
        ...
        ax_diff.plot (*zip(*diff_line), label = "Abweichung" )
